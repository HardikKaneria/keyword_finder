...
from joblib import Parallel, delayed
from services.data_store import get_combined_website_texts, get_combined_website_keywords
...
df_website_keywords = get_combined_website_keywords()

def calculate_website_content_scores_parallel(
    df_scored,
    df_pages=df_pages,
    df_website_keywords=df_website_keywords,
    use_context=False,
    scale_0_to_10=True,
    verbose=True,
    progress_bar=None,
    ...
    if df_pages.empty or df_scored.empty:
        if verbose:
    ...
    try:
        ...
        # Prepare keyword list (fix case)
        keyword_texts = [
            f"{row.get('Source Keyword', '')} {row['Keyword']}".strip()
            if use_context else row["Keyword"]
            for _, row in df_scored.iterrows()
        ]

        if verbose:
            print("🔢 Sample keyword_texts:", keyword_texts[:3])
            print("📰 Sample website_texts:", website_texts[:1])
        ...
        df_scored["semantic_score"] = np.round(sem_scores * 10, 2) if scale_0_to_10 else sem_scores
        ...
        if use_tfidf:
            ...
            df_scored["tfidf_score"] = np.round(tfidf_scores * 10, 2) if scale_0_to_10 else tfidf_scores
        else:
            df_scored["tfidf_score"] = 0

        # === WEBSITE KEYWORD PRESENCE CHECK ===
        if df_website_keywords is not None and not df_website_keywords.empty:
            website_kw_set = set(df_website_keywords["Keyword"].str.lower())
            df_scored["keyword_found_on_site"] = df_scored["Keyword"].str.lower().isin(website_kw_set).astype(int)
        else:
            df_scored["keyword_found_on_site"] = 0

        # === FINAL SCORE BLENDING ===
        df_scored["content_match_score"] = np.round(
            blend_weight_semantic * df_scored["semantic_score"] +
            (1 - blend_weight_semantic) * df_scored["tfidf_score"],
            2
        )
